...
1. **Avvia l’app** (vedi [Avvio](#avvio)) e dalla barra laterale sinistra scegli la pagina:
   - **Sysmac Axes** per tracce degli Sysmac (caricare il csv esportato dal Datatrace del progetto Sysmac)
   - **Sysmac Nodes** per tracce dei nodi servo (caricare il datatrace del nodo di Sysmac)
...
5. **Sysmac Nodes**
   - Stessi passi del grafico 2D di Sysmac Axes: carica il CSV, imposta l’intervallo con lo slider, scegli le variabili, eventualmente **Variabile Scala Y** e **minmax**. I dati sono interpretati con l’header tipico dei nodi (riga 9).
